...
Prenez connaissances de [l'énoncé du TP2](/tp/tp2). Vous aurez besoin d'un petit environnement virtuel hébergé dans [Labinfo](https://labinfo.cegepmontpetit.ca/ui).

Vous pouvez commencer à créer les machines virtuelles dans Labinfo à partir des modèles fournis par le professeur. Suivez les instructions dans le panneau déroulant bleu au début de l'énoncé.

:::info Accès à distance
Il est possible d'accéder à distance à Labinfo en utilisant le service VPN FortiNet offert par le Collège.
:::
