...
Lors de la connexion par le client sur un serveur, ce dernier présente son certificat au client. Le certificat est un fichier qui contient un ensemble d'informations et d'éléments cryptographiques qui rendent sa falsification virtuellement impossible. Parmi les attributs du certificat, il y a:

- Le **SAN** (*subject alternate name*), qui correspond au nom d'hôte du serveur dans la barre d'adresse (entre le `https://` et le `/` suivant, dans l'URL)
...
Il se peut qu'un certificat soit émis par une autorité tierse, qui a été autorisé par une autorité racine. C'est ce qu'on appelle la **chaîne de confiance**. Le certificat porte l'entièreté de la chaîne. Si l'une des autorité de la chaîne n'est pas valide ou reconnue, alors le client ne fera pas confiance au certificat et le navigateur refusera la connexion.
...
- Le nom de domaine (SAN) correspond au nom d'hôte saisi dans la barre d'adresse du navigateur
...
- L'horloge système est à la bonne heure (on pourrait retarder l'heure pour contourner l'expiration du sertificat)
...
- Quand le client envoie une requête au serveur, le cookie associé à ce site est **inclus dans l'en-tête de la requête**
...
### Mythe: Le VPN protège contre les menaces
...
### Mythe: le VPN augmente la performance
...
### Mythe: le VPN protège mes données
...
### Mythe: le VPN protège contre les sites malveillants
